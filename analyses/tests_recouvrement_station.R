...
for(i in levels(recouvrement_substrat$type_substrat)){
  ...
  fit_temp <- glm(formula = filter_data$recouvrement ~ station, family = "poisson",
                  data = filter_data)
...
for(i in orga) {
  ...
  tukey_model <- multcomp::glht(fit_orga[[i]], linfct= multcomp::mcp(station="Tukey"))
  tukey_test[[i]] <- summary(tukey_model)
...
# --- Tests de recouvrement des autres types d'organismes benthiques 
#au sein de chaque type de substrat entre zones géomorphologiques --------------
...
for(i in substrat) {
  ...
  for(j in orga) {
    ...
    fit_temp <- glm(recouvrement_others_in_substrat$recouvrement ~ station, 
